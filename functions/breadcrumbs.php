...
function nova_breadcrumbs( $args = '' ) {
	$args = wp_parse_args( $args, array(
		...
		'show_on_front'     => false,
		'labels'            => array(
			'home'      => esc_html__( 'Home', 'vedbo' ),
			'archive'   => esc_html__( 'Archives', 'vedbo' ),
			'blog'      => esc_html__( 'Blog', 'vedbo' ),
			'search'    => esc_html__( 'Search results for', 'vedbo' ),
			'not_found' => esc_html__( 'Not Found', 'vedbo' ),
			'author'    => esc_html__( 'Author Archives:', 'vedbo' ),
			'day'       => esc_html__( 'Daily Archives:', 'vedbo' ),
			'month'     => esc_html__( 'Monthly Archives:', 'vedbo' ),
			'year'      => esc_html__( 'Yearly Archives:', 'vedbo' ),
		),
	) );
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'vedbo' ), $args['labels']['day'], get_the_date() )
		);
	} // Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'vedbo' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	} // Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'vedbo' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	} // Archive
	else {
		$items[] = sprintf(
